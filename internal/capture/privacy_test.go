...
func TestRedactPreservesOrdinaryHyphenation(t *testing.T) {
	for _, text := range []string{
		...
		"_mask-edit_ and __task-based__ and **disk-backed** checks passed.",
		"[risk-aware](https://example.invalid) and `mask-edit` are unchanged.",
		"A prefix inside an identifier is not a token: widget_ghp_example.",
		"Embedded underscore runs: widget__ghp_example and widget_sk-example.",
		"Unicode underscore words: α_sk-example and 界__ghp_example and e\u0301_sk-example.",
...
func TestRedactSupportedCredentialPrefixes(t *testing.T) {
	...
	wrappers := []struct{ before, after string }{
		...
		{"__", "__"},
		{"___", "___"},
		{"Found _", "_ in the example."},
		{"Found __", "__ in the example."},
		{"*", "*"},
		{"**", "**"},
		{"~~", "~~"},
		{"**_", "_**"},
		{"[_", "_](https://example.invalid)"},
		{"(_", "_),"},
		{"(", ")."},
		{"[", "],"},
		{"{", "}"},
...
func TestRedactMultilineAndMixedText(t *testing.T) {
	for _, tc := range []struct {
		...
		{"markdown single underscore", "Found _sk-proj-synthetic_aB12_ in the example.", excludedLine},
		{"markdown double underscore", "Found __ghp_synthetic__ in the example.", excludedLine},
